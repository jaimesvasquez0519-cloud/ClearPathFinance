...
const schema = z.object({
  ...
  closingDay: z.string().transform((v) => parseInt(v)),
  dueDay: z.string().transform((v) => parseInt(v)),
  cardNetwork: z.enum(['visa', 'mastercard', 'amex', 'other']),
});

type FormInput = { bankName: string; lastFourDigits: string; creditLimit: string; currentBalance: string; closingDay: string; dueDay: string; cardNetwork: 'visa' | 'mastercard' | 'amex' | 'other'; };
...
const NuevaTarjetaModal = ({ onClose }: Props) => {
  ...
  const mutation = useMutation({
    mutationFn: (data: FormData) => api.post('/cards', data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cards'] });
      onClose();
    },
    onError: (err: any) => {
      setServerError(err.response?.data?.error || 'Error al crear la tarjeta');
    },
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex justify-between items-center p-6 border-b border-slate-100">
          <h2 className="text-lg font-bold text-slate-900">Nueva Tarjeta de Crédito</h2>
...
export default NuevaTarjetaModal;
